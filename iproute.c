...
/* 31Jan2023, Maiko, get some 'useful' logging going into the JNOS log */

int ipgarbage (struct iface *ifc, struct mbuf *bp)
{
	static int garbagecount = 0, garbagestage = 1;

	if (!(garbagecount % garbagestage))
	{
		log (-1, "[%s] garbage", ifc->name);
	...
	free_p (bp);	/* no idea what this is, so discard ! */

	return -1;
}

/*
 * 30Jan2023, Maiko (VE4KLM), dual IPv4 / IPv6 stack front end
 *  (get the version right off the bat - then pass it on)
 *
 *    ifc         interface of the incoming packet
 *    bp          incoming packet (data)
 *    rxbroadcast   pass true if this is a broadcast address
 *
 * Replacing 'ip_route' call in config.c with this 'stub' of sorts ...
 *
 */
int ip_preroute (struct iface *ifc, struct mbuf *bp, int rxbroadcast)
{
	char version;

	int retval = 0;

	/* 17Feb2023, Maiko, Need to be strict with prototypes */
	extern int ipv6_route (struct iface*, struct mbuf*, int);

	/* 03Mar2023, Maiko, Don't process IPV6 if no iface */
	extern char *ipv6iface;

	/* Sneak peek the IP version, then send it to appropriate functions */
	version = bp->data[0] >> 4;

	if (version == IPVERSION)
		retval = ip_route (ifc, bp, rxbroadcast);
	...
	else if (version == IPV6VERSION)
	{
		...
		if (ipv6iface)
			retval = ipv6_route (ifc, bp, rxbroadcast);
		else
			free_p (bp);	/* Discard or else we get memory leaks */
	}

	else retval = ipgarbage (ifc, bp);
...
int rxbroadcast;    /* True if packet had link broadcast address */
{
...
#ifndef	IPV6
	/* 30Jan2023, Maiko, Not necessary anymore with pre_route */
	 ...
	 * 18Apr2023, Maiko, Log ipheard now takes a (void*) pointer
...
/* new way: always drop but send a quench according to flag */
    ...
    // log (-1, "ip_route - packet not destined for us");
...
*/
    ...
    // log (-1, "ip_route - checking for packet length");
    ...
    if(ip.length <= iface->mtu){
        ...
        iface->ipsndcnt++;
    	// log (-1, "ip_route - iface->send() [%s]", iface->name);
